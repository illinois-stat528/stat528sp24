...
$$
the prediction for the fitted line at $\hat{p}_i = m$. This method is performed for several $m \in (0,1)$. See [here](https://en.wikipedia.org/wiki/Kernel_smoother) for more details on kernal smoothing.
...
\begin{align}\label{unic}
	\Prob(F(Y|\mathbf{X}=\mathbf{x})\leq s)=s.
\end{align}
...
$$
Now define $H(s; x)$ as the proximal interpolator which maps $s$ to its nearest neighbor on $\Delta(x)$.
